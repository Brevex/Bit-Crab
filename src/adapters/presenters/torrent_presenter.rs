...
{
    println!("Tracker URL: {}", torrent_info
        .announce.as_ref()
        .map(|url| url.as_str())
        .unwrap_or("Tracker URL not found."));

    println!("Length: {}", torrent_info
        .length.
        map_or("File length not found.".to_string(),
               |l| format!("{} bytes", l)
        ));

    println!("Piece Length: {}", torrent_info
        .piece_length
        .map_or("Piece length not found.".to_string(),
                |pl| format!("{}", pl)
        ));
